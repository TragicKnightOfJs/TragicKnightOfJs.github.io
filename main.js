...
function searchMovies(event) {
  ...
  // Call our request
  getMovies("GET", `https://api.themoviedb.org/3/search/movie?api_key=2f54589e7291be847fb969102f1c9bc5&language=en-US&query=${input}&include_adult=true`)
    .then((response) => {
...
function addMoviesList(data, input, containerMovies) {
  ...
  for (let i = 0; i < data.length; i++) {
    // if input value = name movies
    if (data[i].title.indexOf(input)) {
      content += ` 
        <div class="col-md-4">
          <div class="movies-block text-center">
              ${(data[i].poster_path != null) 
                ? `<img class="movies-img" data-item="${i}" src="https://image.tmdb.org/t/p/w150${data[i].poster_path}" alt="Alt">` 
                : `<img  class="movies-img" data-item="${i}"src="http://via.placeholder.com/150x225" alt="Alt" > `} 
        ...
        outputPopup = `
              <div class="col-md-4">
                <div class="movie-popup-img">
                  ...
                  ${(data[i].poster_path != null) 
                    ? `<img class="popup-img" data-item="${i}" src="https://image.tmdb.org/t/p/w150${data[i].poster_path}" alt="Alt">` 
                    : `<img class="popup-img" data-item="${i}" src="http://via.placeholder.com/150x225" alt="Alt" > `} 
